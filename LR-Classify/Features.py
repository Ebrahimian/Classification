__author__ = 'Ahmad'
# Features.py use direct mapping files to generate features of sentences
...
# laod functions, entities, chunks, vocabularies and mappings from map files
def mapping_generator():
    ...
    vocs = []
    uni_symbol_mapping = {}
    files = [os.path.join("C:\Users\Ahmad\Desktop\Classification\LR-Classify\partial", fn) for fn in
             next(os.walk("C:\Users\Ahmad\Desktop\Classification\LR-Classify\partial"))[2]]
    for file in files:
    ...
    # extract the words from train and test set and assign an id to each
    ...
    for chunk in chunks:
        for voc in chunk.split('_'):
            if voc not in vocs:
                vocs.append(voc)  # (TODO: nltk.stem.WordNetLemmatizer().lemmatize(voc))

    return functions, entities, chunks, vocs, uni_symbol_mapping
    ####################################################################################################################


# generate features for entity detection
...
# generate features for function detection
